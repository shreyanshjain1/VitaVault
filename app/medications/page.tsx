                      ...
                      medications.map((medication) => (
                        <DataCard key={medication.id}>
                          ...
                          <div className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                            <p>
                              Schedule: {medication.schedules.map((s) => s.timeOfDay).join(", ") || "—"}
                            </p>
                          ...
                          <div className="mt-5 rounded-2xl border border-border/60 bg-background/40 p-4">
                            <p className="mb-3 text-sm font-medium">Manage medication</p>
                            <form action={updateMedication} className="grid gap-4">
                              <input type="hidden" name="medicationId" value={medication.id} />
                              ...
                              <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                  <Label>Dosage</Label>
                                  <Input name="dosage" defaultValue={medication.dosage} required />
                                </div>

                                <div className="space-y-2">
                                  <Label>Frequency</Label>
                                  <Input name="frequency" defaultValue={medication.frequency} required />
                                </div>

                                {[0, 1, 2].map((index) => (
                                  <div className="space-y-2" key={`${medication.id}-schedule-${index}`}>
                                    <Label>Schedule time {index + 1}</Label>
                                    <Input
                                      name="scheduleTimes"
                                      type="time"
                                      defaultValue={medication.schedules[index]?.timeOfDay ?? ""}
                                      required={index === 0}
                                    />
                                  </div>
                                ))}
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    name="startDate"
                                    type="date"
                                    defaultValue={dateInputValue(medication.startDate)}
                                    required
                                  />
                                </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    name="endDate"
                                    type="date"
                                    defaultValue={dateInputValue(medication.endDate)}
                                  />
                                </div>

                                <div className="space-y-2">
                                  <Label>Prescribing doctor</Label>
                                  <Select name="doctorId" defaultValue={medication.doctorId ?? ""}>
                                    <option value="">Select doctor</option>
                                    {doctors.map((doctor) => (
                                      <option key={doctor.id} value={doctor.id}>
                                        {doctor.name}
                                ...
                                <div className="space-y-2">
                                  <Label>Status</Label>
                                  <Select name="status" defaultValue={medication.status}>
                                    <option value="ACTIVE">Active</option>
                                    <option value="INACTIVE">Inactive</option>
                                    <option value="COMPLETED">Completed</option>
                                  </Select>
                              ...
                              <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
                                <label className="flex items-center gap-2 text-sm font-medium">
                                  <input type="checkbox" name="active" defaultChecked={medication.active} />
                                  Mark as actively taken
                                </label>
                              </div>
                            ...
                            <form action={deleteMedication} className="mt-3">
                              <input type="hidden" name="medicationId" value={medication.id} />
